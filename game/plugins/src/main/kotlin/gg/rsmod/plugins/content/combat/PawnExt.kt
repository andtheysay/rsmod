...
fun Pawn.isInMulti(): Boolean = tile.isMulti(world)
...
fun Pawn.getLastHitByAttr(): Pawn? = attr[LAST_HIT_BY_ATTR]?.get()
...
fun Pawn.dealExactHit(target: Pawn, exactHit: Int, landHit: Boolean, delay: Int, onHit: (PawnHit) -> Unit = {}): PawnHit {
    val hit = if (landHit) {
        target.hit(damage = exactHit, delay = delay)
    } else {
        target.hit(damage = 0, type = HitType.BLOCK, delay = delay)
    }
...
fun Pawn.createProjectile(target: Pawn, gfx: Int, type: ProjectileType, endHeight: Int = -1,extraDelay : Int = 0): Projectile {
    val builder = Projectile.Builder()
            .setTiles(start = tile, target = target)
            .setGfx(gfx = gfx)
            .setHeights(startHeight = type.startHeight, endHeight = if (endHeight != -1) endHeight else type.endHeight)
            .setSlope(angle = type.angle, steepness = type.steepness)
            .setTimes(delay = type.delay + extraDelay, lifespan = type.delay + Combat.getProjectileLifespan(this, target.tile, type)+ extraDelay)
